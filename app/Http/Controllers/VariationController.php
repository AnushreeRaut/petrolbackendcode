...
class VariationController extends Controller
{
    ...
    public function fetchVariationData()
    {
        ...
        $tanks = Tank::all();
        $fuelSalesDetails = FuelSalesDetail::whereDate('created_at', Carbon::today())->get();
        $petrolInvoiceFeeding = PetrolInvoiceFeeding::whereDate('created_at', Carbon::today())->get();

        // Fetch total t_variation grouped by tank_id for today's entries
        // $totalTVariation = DB::table('variations')
        // ->select('tank_id', DB::raw('SUM(t_variation) as total_t_variation'))
        // ->whereDate('created_at', Carbon::today()) // Filter to only today's data
        // ->groupBy('tank_id')
        // ->get()
        // ->keyBy('tank_id'); // Make it easy to access by tank_id
        $totalTVariation = DB::table('variations')
